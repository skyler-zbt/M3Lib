...
int main()
{
    ...
    runner.add("default construct zero-initialises", [] -> TestResult {
        m3::Vec<3, float> v{};
        if (auto r = check(v[0] == 0.0f && v[1] == 0.0f && v[2] == 0.0f); !r) return r;
        return {};
    });
    ...
    runner.add("default construct int is zero", [] -> TestResult {
        m3::Vec<4, int> v{};
        if (auto r = check(v[0] == 0 && v[1] == 0 && v[2] == 0 && v[3] == 0); !r) return r;
        return {};
    });
    ...
    runner.add("component construct vec2", [] -> TestResult {
        ...
        if (auto r = check_float_eq(v[1], 7.0f, 1e-6f); !r) return r;
        return {};
    });

    runner.add("component construct vec3", [] -> TestResult {
        m3::Vec<3, int> v{1, 2, 3};
        if (auto r = check(v[0] == 1 && v[1] == 2 && v[2] == 3); !r) return r;
        return {};
    });
    ...
    runner.add("component construct vec4", [] -> TestResult {
        ...
        if (auto r = check_float_eq(v[3], 4.0f, 1e-6f); !r) return r;
        return {};
    });
    ...
    runner.add("pointer construct vec2", [] -> TestResult {
        ...
        if (auto r = check_float_eq(v[0], 7.0f, 1e-6f); !r) return r;
        if (auto r = check_float_eq(v[1], 13.0f, 1e-6f); !r) return r;
        return {};
    });
    ...
    runner.add("pointer construct vec3", [] -> TestResult {
        int arr[] = {10, 20, 30};
        m3::Vec<3, int> v{arr};
        if (auto r = check(v[0] == 10 && v[1] == 20 && v[2] == 30); !r) return r;
        return {};
    });
    ...
    runner.add("pointer construct roundtrip via value_ptr", [] -> TestResult {
        ...
        for (int i = 0; i < 3; ++i) {
            if (auto r = check_float_eq(internal[i], src[i], 1e-6f); !r) return r;
        }
        return {};
    });
    ...
    runner.add("vec1 x/r/s accessors alias element 0", [] -> TestResult {
        ...
        if (auto r = check_float_eq(v.r(), 7.0f, 1e-6f); !r) return r;
        if (auto r = check_float_eq(v.s(), 7.0f, 1e-6f); !r) return r;
        if (auto r = check_float_eq(v[0], 7.0f, 1e-6f); !r) return r;
        return {};
    });
    ...
    runner.add("operator[] const access", [] -> TestResult {
        ...
        if (auto r = check(v[0] == 5 && v[1] == 5); !r) return r;
        return {};
    });
    ...
    runner.add("negate int vector", [] -> TestResult {
        m3::Vec<2, int> a{5, -3};
        auto c = -a;
        if (auto r = check(c[0] == -5 && c[1] == 3); !r) return r;
        return {};
    });
    ...
    runner.add("int multiply", [] -> TestResult {
        ...
        auto c = a * 3;
        if (auto r = check(c[0] == 6 && c[1] == 9 && c[2] == 12 && c[3] == 15); !r) return r;
        return {};
    });
    ...
    runner.add("int division truncation", [] -> TestResult {
        ...
        m3::Vec<2, int> a{5, 7};
        auto c = a / 2;
        if (auto r = check(c[0] == 2 && c[1] == 3); !r) return r;
        return {};
    });
    ...
    runner.add("format vec3 int", [] -> TestResult {
        m3::Vec<3, int> v{1, 2, 3};
        auto s = std::format("{}", v);
        if (auto r = check(s == "[1, 2, 3]"); !r) return r;
        return {};
    });
    ...
    runner.add("format vec1 single element", [] -> TestResult {
        m3::Vec<1, int> v{42};
        auto s = std::format("{}", v);
        if (auto r = check(s == "[42]"); !r) return r;
        return {};
    });
    ...
    runner.add("format vec4", [] -> TestResult {
        ...
        auto s = std::format("{}", v);
        if (auto r = check(s == "[10, 20, 30, 40]"); !r) return r;
        return {};
    });
    ...
    runner.add("format vec2 float produces bracketed output", [] -> TestResult {
        ...
        auto s = std::format("{}", v);
        if (auto r = check(s.size() > 0 && s[0] == '[' && s.back() == ']'); !r) return r;
        return {};
    });
