...
constexpr bool was_signalled(int status) { return (status & 0x7f) != 0; }
constexpr int term_signal(int status) { return status & 0x7f; }
...
int main() {
    ...
    runner.add("Vec<3>[3] OOB read aborts", [] -> TestResult {
        if (!expect_abort([] { const m3::Vec<3, float> v{1.f, 2.f, 3.f}; auto x = v[3]; }))
            return std::unexpected("no abort on OOB read");
        return {};
    });

    runner.add("Vec<2> far OOB read aborts", [] -> TestResult {
        if (!expect_abort([] { const m3::Vec<2, int> v{0}; auto x = v[100]; }))
    ...
    runner.add("Vec<4>[0] and [3] valid access passes", [] -> TestResult {
        ...
        if (auto r = check(v[0] == 1); !r) return r;
        if (auto r = check(v[3] == 4); !r) return r;
        return {};
    });
    ...
    runner.add("Vec<2> int div-by-zero aborts", [] -> TestResult {
        if (!expect_abort([] { m3::Vec<2, int> v{10, 20}; auto r = v / 0; }))
